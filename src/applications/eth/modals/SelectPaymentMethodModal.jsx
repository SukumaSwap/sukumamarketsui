...
import { getPaymentMethods, getTextCount, getTheme } from "../../../app/appFunctions"
...
import {  PAYMENT_METHODS } from "../../../app/appconfig"
import { useDebouncedValue, useMediaQuery } from '@mantine/hooks';

import FlipMove from 'react-flip-move';


const SelectPaymentMethodModal = ({ select, selected, open, closeModal }) => {

    const objs = PAYMENT_METHODS;
    ...
    return (
        <Modal overflow='inside' lockScroll
            opened={open}
            // title={<Title order={2}>Select Payment Method</Title>} 
            ...
            <Box className='custom-h' p="md">
                <Group position='apart'>
                    <Title order={2}>Select Payment Method</Title>
            ...
            <Box className='custom-body' py="xs">
                <Box px="md" style={{ height: "150px", background: "rd" }}>
                    ...
                    <Title order={5} mb="xs">Common methods</Title>
                    <Carousel slideGap={10} align="start" slideSize="40%">
                        {
                            getPaymentMethods(["M-Pesa"]).map((item, i) => (
...
const SelectAsset = ({ asset, select, selected }) => {
    return (
        <Paper mb="xs" px="md" sx={theme => ({
            background: selected?.name === asset?.name ? getTheme(theme) ? theme.colors.dark[6] : theme.colors.gray[1] : "transparent",
            cursor: "pointer",
            pointerEvents: selected?.name === asset?.name ? "none" : "all",
            ":hover": {
            ...
            <Group>
                ...
                })}>{asset?.name?.substring(0, 1)}</Avatar>
                <Stack spacing={-10}>
                    <Text size="md"><b>${asset?.name}</b></Text>
