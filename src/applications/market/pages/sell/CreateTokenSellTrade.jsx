    ...
    const initiateChat = () => {
        ...
        if (contract && conn && contract.add_sell_chat) {
            ...
            const chat = {
                ...
                payment_msg: `<b>${offerDetails.offerer?.id}</b> will pay <b>${conn.getAccountId()}</b> <b>${offerDetails?.currency} ${formatNumber(Math.round(pay))}</b> for <b>${formatNumber(receive)} ${offerDetails?.token?.name}</b>. <br/>This amount is approximately <b>$${formatNumber(Math.round(pay / CURRENT_USD_PRICE_IN_KES))}</b>`
            }
            console.log(chat)
            setLoading(true)
