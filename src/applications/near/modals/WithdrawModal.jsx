...
import { Button, Group, Modal, Paper, Stepper, Text, TextInput, Title, NumberInput, ScrollArea, Center, Stack, Avatar, useMantineTheme, Loader } from '@mantine/core';
...
const WithdrawModal = () => {
    ...
    const nearWithdraw = () => {
        ...
        if (amtToDeposit === 0) {
            ...
            const amt = new BigNumber(amtToDeposit).multipliedBy(1e24).toFixed()
            contract.withdraw_near({
    ...
    const withdraw = () => {
        if (token === "near") {
            ...
            // tokenWithdraw()
            console.log("Withdrawing asset: ", token)
        }
    }
            ...
            <Modal opened={open}
                ...
                <Group mt="md" position='center'>
                    ...
                    {step === 1 && (
                        <Button radius="xl" color="violet"
                            style={{ height: "42px" }}
                            rightIcon={loading ? <Loader variant='oval' /> : <IconCurrencyDollar />}
                            onClick={withdraw}
                        >
                            Withdraw
                        </Button>
